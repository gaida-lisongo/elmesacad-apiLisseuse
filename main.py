...
def _cors_origins(settings: Settings) -> list[str]:
    ...
    origins = [
        ...
        "http://127.0.0.1:5173",
        "http://127.0.0.1:8000",
        # Ton VPS OVH (Indispensable pour la liseuse)
        "http://vps-90d3200d.vps.ovh.net:8080",
    ]
    
    # Ajout dynamique via les paramètres d'environnement (GitHub Pages, etc.)
    if settings.cors_github_pages_origin:
        origins.append(settings.cors_github_pages_origin.rstrip("/"))
...
@app.get("/favicon.ico", include_in_schema=False)
async def favicon():
    """Gestion du favicon pour éviter les 404 dans les logs."""
    return Response(status_code=204)


@app.get("/health")
async def health():
    """Route de santé pour Render/Monitoring."""
    return {"status": "ok", "service": "ELMESACAD-API"}
